...
activateTradingKey(handler::CoinigyHandler, authId::Int) = getResult(handler, "$(handler.base_url)/activateTradingKey", Dict("auth_id" => authId, "auth_trade" => 1))
addOrder(handler::CoinigyHandler, authId::Int, exchId::Int, mktId::Int, orderTypeId::Int, priceTypeId::Int, price::Float64, quantity::Float64) = getResult(handler, "$(handler.base_url)/addOrder", Dict("auth_id" => authId, "exch_id" => exchId, "mkt_id" => mktId, "order_type_id" => orderTypeId, "price_type_id" => priceTypeId, "limit_price" => price, "order_quantity" => quantity))
